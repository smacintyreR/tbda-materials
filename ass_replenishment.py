...
# Optimization function
prob_pois += lpSum([store_variables_pois[i]*margin[i] - store_variables_pois[i]*transport[i] for i in lstore]), "Total Revenue"

# Restrictions

# Add restriction for total available stock
prob_pois += lpSum([store_variables_pois[i] for i in lstore]) == initial_stock, "StockAvailable"
